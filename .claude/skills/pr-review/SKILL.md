...
- **무대응 금지** — 작성자가 답글을 보냈는데 내버려두면 auto-merge가 트리거되지 않는다(resolve 안 됨). 반드시 동의·재반박·종결 셋 중 하나로 처리
...
다음에 해당하면 회차와 무관하게 종결하지 말고 사용자(인간)에게 보고하고 중단한다:

- 원래 등급이 P1인 항목 (실제 버그·기능 회귀)
- 보안 취약점·데이터 손실·컴플라이언스/법적 요구사항
- 빌드/CI 실패가 명확히 예상되는 변경

결과 테이블에 `BLOCKED (P1 합의 실패)` 표기. resolve도 merge도 하지 않는다.
...
| 부당하지만 P2/P3 + 내 발언 ≥ 2회        | 본 §8.3.1 종결 답글 + resolve                        |
| 부당하고 P1·보안·CI 우려                | `BLOCKED` 표기, 사용자 보고, resolve·merge 모두 보류 |
| 부당하지만 내 발언 < 2회 + 새 근거 있음 | 재반박 (라운드 한도 내)                              |

**resolve를 빠뜨리지 말 것** — 작성자는 어떤 경우에도 자기 PR 스레드를 resolve하지 않으므로(§8.4), 이 단계에서 리뷰어가 resolve하지 않으면 auto-merge가 영영 트리거되지 않는다.
...
재분석 → 부당 + 내 발언 ≥ 2회 + P2/P3 → 종결 답글 + resolve
재분석 → 부당 + 내 발언 ≥ 2회 + P1/보안/CI → BLOCKED, 사용자 보고
```
...
- **ALWAYS 의견 충돌 종결 프로토콜** — 같은 스레드에서 내 발언이 2회에 도달했는데도 합의 안 되면, 새 반박 대신 "작성자 판단을 존중하겠습니다" 종결 답글 1회 + resolve로 마무리(§8.3.1). P1·보안·데이터 손실·CI 실패 우려는 `BLOCKED`으로 사용자에게 보고하고 resolve·merge 모두 보류
